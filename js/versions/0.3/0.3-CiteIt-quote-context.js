...
jQuery.fn.quoteContext = function() {
	...
	jQuery(this).each(function(){
		...
		if( jQuery(this).attr("cite") ){
			...
			var youtube_icon = "";
			var embed_html = ''; //embed_code(citing_url);
			...
			if (cited_url.length > 3){
				...
				function add_quote_to_dom(tag_type, json ) {
					if ( tag_type == "q"){
						...
						context_before.before("<div class='quote_arrows context_up' id='context_up_" + json['sha256'] + "'> \
							<a href=\"javascript:toggle_quote('before', 'quote_before_" + json['sha256'] + "');\">&#9650;</a> " + 
								"<a href=\"javascript:toggle_quote('before', 'quote_before_" + json['sha256'] + "');\">" +
							youtube_icon + "</a></div>");
						}
						

						if( json['cited_context_after'].length > 0){
						context_after.after("<div class='quote_arrows context_down' id='context_down_" + json['sha256'] +"'> \
...
function embed_code(url){
	...
	u = urlParser.parse(url);
	if (u){
		console.log("Printing: u");
		console.log(u);
		//if (u.hasOwnProperty('provider')) {
		//	provider = u.provider;
		//}
	...
	if (provider == 'youtube') {
		if ('params' in u) {
			start_time = u.params.start;
		}

		embed_html = '<iframe width="560" height="315" src="' + url + '" frameborder="0" allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>';  
	}
	...
	// var url = 'https://www.youtube.com/embed/1NqRbBvujHY?start=399';
}